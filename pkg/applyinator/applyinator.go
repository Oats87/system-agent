...
func NewApplyinator(workDir string, preserveWorkDir bool, appliedPlanDir, markerDir string, imageUtil *image.Utility) *Applyinator {
	return &Applyinator{
		mu:              &sync.Mutex{},
		workDir:         workDir,
...
func (a *Applyinator) Apply(ctx context.Context, input ApplyInput) (ApplyOutput, error) {
	...
	restartPendingMarkerFilePath := filepath.Join(a.markerDir, restartPendingMarkerFile)
	...
	applyinatorActiveMarkerFilePath := filepath.Join(a.markerDir, applyinatorActiveMarkerFile)
